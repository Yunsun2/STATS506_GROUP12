...
Multinomial logistic regression is an extension of binary logistic regression that dependent variables have more than two categories. The model is used to predict the probabilities of the different outcomes. It is important and useful since we can predict and then control the outcome by some degrees.
...
## Our Model
Our task is to model two equations below using R, Stata and SAS with Iris data set. 
...
It may be because the coefficients of the former are not significant at all while some of the coefficients of the latter are significant. We will figure it out more later.
